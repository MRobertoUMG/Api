...
with open(input('Seleccione un archivo CSV '), newline='') as csvfile:
    ...
    for fila in lector_csv:
        Tree.insert(fila['Provider Name'])

endTimeLoad = time.time()
totalTimeload = (endTimeLoad - startTimeLoad) * 1000
print("****Tiempo tardado para cargar la informacion en el árbol binario es: ", totalTimeload , "milisegundos ****")'''

#nodo = nodoArbol()
'''startTimePrint = time.time()
Tree.mostrar(Tree.raiz)
endTimePrint = time.time()
totalTimePrint = (endTimePrint - startTimePrint) * 1000
print("****Tiempo tardado para imprimir la informacion en el árbol binario es: ", totalTimePrint , "milisegundos ****")'''